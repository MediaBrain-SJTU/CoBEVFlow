...
class PillarVFE(nn.Module):
    def __init__(self, model_cfg, num_point_features, voxel_size,
        ...
        self.voxel_x = voxel_size[0] # 每个 voxel 的长/宽/高
        self.voxel_y = voxel_size[1]
        self.voxel_z = voxel_size[2]
        self.x_offset = self.voxel_x / 2 + point_cloud_range[0] # 初始voxel的几何中心的 x/y/z 坐标
    ...
    def get_paddings_indicator(actual_num, max_num, axis=0):
        ...
        paddings_indicator = actual_num.int() > max_num  # array: (M, max_num_points), 
    ...
    def forward(self, batch_dict):
        ...
        voxel_features, voxel_num_points, coords = \
            ...
            batch_dict['voxel_coords'] # voxel_features: x, y, c, r

        points_mean = \
            voxel_features[:, :, :3].sum(dim=1, keepdim=True) / \
            voxel_num_points.type_as(voxel_features).view(-1, 1, 1)
        f_cluster = voxel_features[:, :, :3] - points_mean  # (M, max_num_points, 3) 每个点与 voxel 中 [重心] 的坐标偏移量 x_c, y_c, z_c
        # coords: 存放每个 voxel 的顺序 -> zyx
        f_center = torch.zeros_like(voxel_features[:, :, :3]) # (M, max_num_points, 3) 每个点与 voxel [center] 的坐标偏移量 x_c, y_c, z_c
        ...
        features = torch.cat(features, dim=-1) # (M, max_num_points, 10)

        voxel_count = features.shape[1]
        mask = self.get_paddings_indicator(voxel_num_points, voxel_count,
                                           axis=0)  # (M, max_num_points)
        mask = torch.unsqueeze(mask, -1).type_as(voxel_features)  # (M, max_num_points, 1) float
        features *= mask # 每个voxel块中的补出来的点都置零, (M, 32, 10)
        for pfn in self.pfn_layers:
            features = pfn(features)
        features = features.squeeze()  # (M, 64)
