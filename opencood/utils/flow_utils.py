...
def generate_flow_map_and_mask(flow, bbox_list, scale=1.25, shape_list=None, align_corners=False, file_suffix=""):
    """
    Parameters
    -----------
...
def find_id_difference(obj_A, obj_B, id_A, id_B):
    ...
    # 计算差值
    diff = B_common[:, :3] - A_common[:, :3]

    return diff, A_common

def generate_flow_map_szwei(object_stack, object_id_stack, cav_lidar_range, voxel_size, past_k=1):
    '''
    SizheWei@2023/04/20 
    ...
    shape_list = torch.tensor([64, 200, 704])
    ...
    return flow_map.squeeze(0).permute(2, 0, 1).numpy()
...
def generate_flow_map(object_stack, object_id_stack, cav_lidar_range, voxel_size, past_k=1):
    ...
    prev_box = box_list_to_dict(object_stack[past_k], object_id_stack[past_k], cav_lidar_range, voxel_size)
...
def global_coord_to_bev(box, cav_lidar_range, voxel_size, out_size_factor=1):
...
def init_bev_map(cav_lidar_range, voxel_size, out_size_factor=1):
...
def disp_in_box(bev_map, points, prev_box, curr_box):
    ...
    # idx = point_in_hull_fast(points, curr_bounding_box)
    pc_in_box = points[idx]
    disp = calc_displace_vector(pc_in_box, prev_bounding_box, curr_bounding_box)
    # disp = calc_displace_vector(pc_in_box, curr_bounding_box, prev_bounding_box)
