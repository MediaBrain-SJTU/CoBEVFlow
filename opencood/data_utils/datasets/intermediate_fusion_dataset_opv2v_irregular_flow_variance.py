# -*- coding: utf-8 -*-
# Author: sizhewei @ 2023/4/30
# License: TDG-Attribution-NonCommercial-NoDistrib

"""
two stage flow update framework
带有variance的irregular dataset
"""
...
from opencood.data_utils.datasets import basedataset, intermediate_fusion_dataset_opv2v_irregular_flow_new
...
class IntermediateFusionDatasetIrregularFlowVariance(intermediate_fusion_dataset_opv2v_irregular_flow_new.IntermediateFusionDatasetIrregularFlowNew):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetIrregularFlowVariance, self).__init__(params, visualize, train)

        # random sample range 
        if 'ir_range' in params:
            self.sample_range = params['ir_range']
        else:
            self.sample_range = 0

        self.sample_anchor = list(np.array([-0, -4, -8]) - int(self.sample_interval_exp))

        print("=== Random Sampling Range Scale = {}. ===".format(self.sample_range))
            ...
            for i in range(self.k):
                ...
                if data[cav_id]['ego']:             # ego sample_interval = E(B(n, p))
                    if i == 0: # ego-past-0 与 ego-curr 是一样的
                        ...
                        continue
                    this_sample_stamp_idx = curr_timestamp_idx + self.sample_anchor[i]
                    sample_interval = latest_sample_stamp_idx - this_sample_stamp_idx
                    latest_sample_stamp_idx = this_sample_stamp_idx
                else:                               # non-ego sample_interval ~ B(n, p)
                    if i==0:  # if non-ego-past-0, must late than current frame # TODO:
                        unit_offset = random.randint(-self.sample_range, 1)
                    else:
                        unit_offset = random.randint(-self.sample_range, self.sample_range)
                    this_sample_stamp_idx = curr_timestamp_idx + self.sample_anchor[i] + unit_offset
                    sample_interval = latest_sample_stamp_idx - this_sample_stamp_idx
                    latest_sample_stamp_idx = this_sample_stamp_idx
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            processed_data_dict['ego'].update({'avg_time_delay':\
                avg_time_diff})
            tmp_var = np.var(tmp_past_k_time_diffs, axis=1) # (N, )
