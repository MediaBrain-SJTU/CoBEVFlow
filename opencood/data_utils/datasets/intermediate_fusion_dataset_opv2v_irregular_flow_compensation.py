...
3) gt flow
on irregular OPV2V
This dataloader is just for debug, to viz :
    past_0 feature + flow map == curr feature ?
"""
...
class IntermediateFusionDatasetIrregularFlowCompensation(intermediate_fusion_dataset_opv2v_irregular.IntermediateFusionDatasetIrregular):
    """
    ...
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetIrregularFlowCompensation, self).__init__(params, visualize, train)
        self.k = self.k + 1 # 用来在 past_k 中添加 current frame
        print("=== OPV2V-Irregular Multi-sweep dataloader (Flow Compensation Ver.) for intermediate fusion with non-ego cavs' past {} frames collected initialized! Expectation of sample interval is {}. ### {} ###  samples totally! ===".format(self.k-1, self.binomial_n * self.binomial_p, self.len_record[-1]))
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            for i in range(self.k):
                # ego-past-0 与 ego-curr 是一样的, 用于feature监督
                if i == 0: 
                ...
                if data[cav_id]['ego']:             # ego: do not need past frame
                    if i == 1: # ego-past-1 与 ego-curr 是一样的
                        data[cav_id]['past_k'][i] = data[cav_id]['curr']
                        continue
                    ...
                    if sample_interval==0:
                        if i==1: # 检查past 0 的实际时间是否在curr 的后面
    ...
    def __getitem__(self, idx):
        ...
        past_k_lidar_poses = np.array(past_k_pose_stack).reshape(-1, self.k, 6)  # (Nx(k-1)+1, 6)
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, idx):
        ...
        # generate flow, from past_1 and curr
        prev_object_id_stack = {}
        prev_object_stack = {}
        for t_i in range(2):
            split_part = selected_cav_base['past_k'][1] if t_i == 0 else selected_cav_base['curr']
            object_bbx_center, object_bbx_mask, object_ids = \
                self.generate_object_center([split_part], selected_cav_base['past_k'][1]['params']['lidar_pose'])
