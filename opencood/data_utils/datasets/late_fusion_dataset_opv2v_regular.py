...
"""
Dataset class for Late fusion with past k frames on regular OPV2V
...
from opencood.data_utils.datasets import basedataset, intermediate_fusion_dataset_opv2v_multisweep
...
class LateFusionDatasetRegular(intermediate_fusion_dataset_opv2v_multisweep.IntermediateFusionDatasetMultisweep):
    """
    This class is for late fusion where each vehicle transmit the
    detection outputs to ego.
    """
    def __init__(self, params, visualize, train=True):
        super(LateFusionDatasetRegular, self).__init__(params, visualize, train)

        print("=== OPV2V-Regular Multi-sweep dataset for late fusion with non-ego cavs' past {} frames collected initialized! Time delay is {}. ### {} ###  samples totally! ===".format(self.k, self.tau, self.len_record[-1]))
    ...
    def get_item_single_car_test(self, selected_cav_base):
        ...
        for part in ['past_k', 'curr']:
            processed_part = {}
            if part=='past_k':
                processing_base = selected_cav_base['past_k'][0] # 只用距离最近的过去帧
            else:
