...
class IntermediateFusionDataset_outage(intermediate_fusion_dataset.IntermediateFusionDataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDataset_outage, self). \
    ...
    def retrieve_base_data(self, idx):
        ...
        # find the idx of past frames
        past_timestamp_list = []
        for iter in range(self.k):
            past_timestamp = timestamp_index - self.k + iter
            if past_timestamp < 0:
                past_timestamp = 0
            past_timestamp_list.append(past_timestamp)
        ...
        for i in range(len(past_timestamp_list)):
            timestamp_index = past_timestamp_list[i]
            timestamp_key = self.return_timestamp_key(scenario_database, timestamp_index)
            past_feature = OrderedDict()
            for cav_id, cav_content in scenario_database.items():
                past_feature[cav_id] = OrderedDict()
                past_feature[cav_id]['ego'] = cav_content['ego']
                past_feature[cav_id]['params'] = load_yaml(cav_content[timestamp_key]['yaml'])
                past_feature[cav_id]['lidar_np'] = pcd_utils.pcd_to_np(cav_content[timestamp_key]['lidar'])
    ...
    def __getitem__(self, idx):
        ...
        '''     
        past_processed_features_list = self.get_past_processed_features_opv2v(past_features)
    ...
    def get_past_processed_features_opv2v(self, past_features):
        past_processed_features_list = []

        for i in range(len(past_features)):
            temp_dict = add_noise_data_dict(past_features[i],self.params['noise_setting'])
            processed_data_dict = OrderedDict()
            processed_data_dict['ego'] = {}
            
            for cav_id, cav_content in temp_dict.items():
                if cav_content['ego']:
                    ego_id = cav_id
                    ego_lidar_pose = cav_content['params']['lidar_pose']
                    ego_lidar_pose_clean = cav_content['params']['lidar_pose_clean']
            ...
            for cav_id, selected_cav_base in temp_dict.items():
            # check if the cav is within the communication range with ego
                lidar_pose_list.append(selected_cav_base['params']['lidar_pose']) # 6dof pose
                cav_id_list.append(cav_id)
            
            for cav_id in cav_id_list:
                selected_cav_base = temp_dict[cav_id]
            ...
            pairwise_t_matrix = self.get_pairwise_transformation(temp_dict, self.max_cav)
            ...
            past_processed_features_list.append(processed_data_dict)

        return past_processed_features_list
    ...
    def collate_batch_train_past(self, batch):

        past_features = {}
        past_lidar_features_list = []
        past_lidar_poses_list = []
        past_transmatrix_list = []
        record_len = {}

        for i in range(len(batch)):
            past_list = batch[i]['ego']['past_features']
            record_len_temp = []
            for j in range(self.k):
                # print(len(past_list[j]['ego']['processed_lidar']['voxel_coords']))
                past_lidar_features_list.append(past_list[j]['ego']['processed_lidar'])
                past_lidar_poses_list.append(past_list[j]['ego']['lidar_poses'])
                past_transmatrix_list.append(past_list[j]['ego']['pairwise_t_matrix'])
                record_len_temp.append(past_list[j]['ego']['cav_num'])
            record_len[i] = record_len_temp
        # record_len = torch.from_numpy(np.array(record_len))
        lidar_pose = torch.from_numpy(np.concatenate(past_lidar_poses_list, axis=0))
        pairwise_t_matrix = torch.from_numpy(np.array(past_transmatrix_list))
        merged_feature_dict = self.merge_features_to_dict(past_lidar_features_list)

        # print(len(merged_feature_dict['voxel_coords']))

        past_lidars = self.pre_processor.collate_batch(merged_feature_dict)
        ...
        past_features.update(
            {'processed_lidar':past_lidars,
            'lidar_poses': lidar_pose,
            'pairwise_t_matrix': pairwise_t_matrix,
    ...
    def collate_batch_train(self, batch):
        ...
        # past_lidar_feature_dict = self.merge_features_to_dict(past_lidar_features_list)

        # [sum(record_len), C, H, W]
        processed_lidar_torch_dict = \
            self.pre_processor.collate_batch(merged_feature_dict)
        # past_lidar_torch_dict = self.pre_processor.collate_batch(past_lidar_feature_dict)
        # [2, 3, 4, ..., M], M <= max_cav
        ...
        past_lidar_torch_dict = self.collate_batch_train_past(batch)
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                   ...
                                   'past_features': past_lidar_torch_dict})
    ...
    def get_pairwise_transformation_past(self, base_data_dict, max_cav):
        """
        Get pair-wise transformation matrix accross different agents.
