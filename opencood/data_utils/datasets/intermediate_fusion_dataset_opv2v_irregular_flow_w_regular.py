...
对比 intermediate_fusion_dataset_opv2v_irregular_flow , 区别:
该类含有 with_regular 参数, 用于控制是否使用绝对regular的sample.
"""
...
class IntermediateFusionDatasetIrregularFlowWRegular(basedataset.BaseDataset):
    """
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, idx):
        ...
        prev_object_stack = {}
        for t_i in range(2):
            split_part = selected_cav_base['past_k'][0] if t_i == 0 else selected_cav_base['curr']
            object_bbx_center, object_bbx_mask, object_ids = \
