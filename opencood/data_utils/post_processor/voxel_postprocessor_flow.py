...
class VoxelPostprocessorFlow(BasePostprocessor):
    def __init__(self, anchor_params, train):
        super(VoxelPostprocessorFlow, self).__init__(anchor_params, train)
    ...
    def flow_post_process(self, data_dict, output_dict, num_sweeps=2):
        """
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            box_results[cav_id] = OrderedDict()
            box_results[cav_id].update({
                'past_k_time_diff' : data_dict[cav_id]['past_k_time_diff']
            })
            # the transformation matrix to ego space
            # transformation_matrix = cav_content['transformation_matrix'] # no clean
            transformation_matrix = cav_content['trans_mat_past_2_past0_torch']

            # (H, W, anchor_num, 7)
            anchor_box = cav_content['anchor_box']

            # classification probability
            prob = output_dict[cav_id]['psm']  # k,2,100,352
            prob = torch.sigmoid(prob.permute(0, 2, 3, 1))
            prob = prob.reshape(self.k, -1) # (k, HxWx2)

            # regression map
            reg = output_dict[cav_id]['rm'] # (k,14,100,352)
            ...
            for i in range(self.k):  # 遍历所有的时间戳
                box_results[cav_id][i] = OrderedDict()
    ...
    def post_process_fuse_updated_frame(self, data_dict, output_dict):
        """
        Process the flow-updated box from cavs to ego-cav view.
        Step2: project the bounding boxes to ego space.
        Step3: NMS
        ...
        output_dict : dict for each cav at each time
            {
                'ego' / cav_id : {
                    'past_k_time_diff' : 
                    ['comp'] {
        ...
        Returns
        -------
        pred_box3d_tensor : bbx fused from different cavs at ego-curr view
            torch.tensor(N, 8, 3)
        ...
        for cav_id, cav_content in output_dict.items():
            ...
            transformation_matrix = data_dict[cav_id]['trans_mat_past0_2_curr_torch']  # 4, 4

            box_3d = cav_content['comp']['pred_box_3dcorner_tensor']
            scores = cav_content['comp']['scores']
            box_3d_delay = cav_content[0]['pred_box_3dcorner_tensor']
            if box_3d.shape[0] != 0:
                projected_boxes3d = box_utils.project_box3d(box_3d, transformation_matrix)
                pred_box3d_list.append(projected_boxes3d)
                scores_list += scores
            if box_3d_delay.shape[0] != 0:
                projected_boxes3d_delay = box_utils.project_box3d(box_3d_delay, transformation_matrix)
                delay_box3d_list.append(projected_boxes3d_delay)

        pred_box3d_tensor = torch.vstack(pred_box3d_list)
        delay_box3d_tensor = torch.vstack(delay_box3d_list)
        scores = torch.hstack(scores_list)
        assert pred_box3d_tensor.shape[0] == scores.shape[0]
        ...
        delay_box3d_tensor = delay_box3d_tensor[keep_index]
        ...
        delay_box3d_tensor = delay_box3d_tensor[mask, :, :]

        assert scores.shape[0] == pred_box3d_tensor.shape[0]

        
        return pred_box3d_tensor, scores, delay_box3d_tensor
